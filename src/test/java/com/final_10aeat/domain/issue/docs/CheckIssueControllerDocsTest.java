...
import com.final_10aeat.domain.articleIssue.controller.CheckIssueController;
...
public class CheckIssueControllerDocsTest extends RestDocsSupport {

    private final IssueCheckService issueCheckService = Mockito.mock(
        IssueCheckService.class);

    @Override
    public Object initController() {
        objectMapper = new ObjectMapper();
        return new CheckIssueController(issueCheckService);
    }
    ...
    void testIssueDetail() throws Exception {
        // given
        IssueDetailResponseDto responseDto = new IssueDetailResponseDto("이슈 제목",
            "이슈 내용");

        // when
        when(issueCheckService.getIssueDetail(any(Long.class))).thenReturn(responseDto);
